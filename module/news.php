...
$cat = killInjection(isset($_REQUEST['cat']) ? $_REQUEST['cat'] : '' );
if ($cat=='') $cat = $parentRecord['parent'];
$per_page = 10;
$p=0;
if (isset($_REQUEST['p']) && $_REQUEST['p']!='') $p=killInjection($_REQUEST['p']);
...
<table align="center" border="0" width="98%" cellspacing="0" cellpadding="0">
	<tr>
       <td width="19%"><a href="./?frame=news_detail&id=<?=$row['id']?>">
	   	<img src="<?=$row['image']?>" width="122" height="75" border="0" hspace="5"/></a></td>
      <td width="81%" align="left" valign="top"><span class="font_blue">
	  		<a href="./?frame=news_detail&id=<?=$row['id']?>" class="link_menu_sp"><?=$row['name']?></a></span><br />
        <samp class="font_xam"><?=$row['detail_short']?></samp></td>
    </tr>
