...
<table border="1" cellpadding="2" bordercolor="#C9C9C9" width="100%">
	<tr>
	  <th width="27" class="title"><input type="checkbox" name="chkall" onClick="chkallClick(this);"></th>
		...
		<th width="67" class="title"><a class="title" href="<?=getLinkSort(15)?>">Danh mục con</a></th>
...
while($row=mysql_fetch_array($result)){
	...
	<tr>
		...
		<td bgcolor="<?=$color?>" class="smallfont"><a href="./?act=product_category_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>"><?=$row['name']?></a></td>
		...
		<td bgcolor="<?=$color?>" class="smallfont" align="center">
			<input type="checkbox" disabled <?=$row['child']>0?'checked':''?>>
		</td>
