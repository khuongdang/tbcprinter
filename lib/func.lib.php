...
function getRecord($table, $where='1=1'){
	...
	$result = mysql_query("select * from $table where $where limit 1");
...
function countRecord($table,$where=""){
    ...
    if ($table=="") return false;
    if ($where=="") $where="1=1";
	$result = mysql_query("select count(*) as cnt from $table where $where");
...
//--------------------Unit-------------------------------------------------------------------------------------------------------
function comboUnit($index, $lang="vn", $name="cmbUnit", $class="textbox"){
	$arrValue  = array('0','1');
	$arrTextVN = array('VND','USD');
	$arrTextEN = array('VND','USD');
	$arrText = $lang=="vn"?$arrTextVN:$arrTextEN;
	$firstValue = $lang=="vn"?"[Chọn giá]":"[Select price]";
	$out = '';
	$out .= '<select name="'.$name.'" id="'.$name.'" class="'.$class.'">';
	$out .= '<option value="-1">'.$firstValue.'</option>';
	for($i=0; $i<count($arrValue); $i++){
...
function mail_smtp($from,$to,$subject,$body,$html=0){
	...
	$smtp->pop3_auth_host="pmvietnam.vn";           /* Set to the POP3 authentication host if your SMTP server requires prior POP3 authentication */
	$smtp->user="customer@pmvietnam.vn";                     /* Set to the user name if the server requires authetication */
	$smtp->realm="";                    /* Set to the authetication realm, usually the authentication user e-mail domain */
	$smtp->password="customer";                 /* Set to the authetication password */
